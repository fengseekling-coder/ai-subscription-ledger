...
export function parseUsagePaymentDate(row: SubscriptionRow): string {
...
export function parseUsageToBill(row: SubscriptionRow): Bill | null {
  const u = String(row.usage || "").trim();
...
export function applyRowMigrations(rows: SubscriptionRow[]): boolean {
  void rows;
  return false;
}

export function ensureSubscribedAtFromRows(rows: SubscriptionRow[], bills: Bill[]): boolean {
  let migrated = false;
...
export function ensureBillsFromRows(rows: SubscriptionRow[], bills: Bill[]): Bill[] {
...
export function hydrateImportedState(parsed: {
  budget?: unknown;
  rows?: unknown[];
  bills?: unknown[];
}): AppState {
...
export function normalizePersistedState(state: AppState): AppState {
  const rows = state.rows.map((r) => normalizeRow(r));
  let migrated = applyRowMigrations(rows);
  const bills = ensureBillsFromRows(rows, (state.bills || []).map(normalizeBill));
  if (ensureSubscribedAtFromRows(rows, bills)) migrated = true;
  void migrated;
  return {
    budget: Number(state.budget) || 500,
    rows,
    bills,
  };
}
